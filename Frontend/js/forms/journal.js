...
import { default as renderJournal } from "../journal.js";
...
async function send() {
  ...
  let response = await get(
    "http://localhost:120/SI/hello/create_journal",
    "POST",
    formData
  );
  return response;
...
function errorGestion(response) {
  ...
  if (response == 2) {
    createMessage("Le journal que vous voulez créer éxiste déja", "error1");
  }
  if (response == 0) {
    remove();
    window.setTimeout(() => {
...
function createFormContainer() {
  ...
  response.innerHTML = `
    ...
    <form class="form__container__form">
      <h1 class="form__container__form__title">Journal</h1>
      <input
        autocomplete="off"
        type="text"
