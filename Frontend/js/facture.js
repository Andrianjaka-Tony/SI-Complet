...
import { default as renderFormFacture } from "./forms/facture.js";
...
function createFacture(facture) {
  ...
  response.querySelector(".modify").addEventListener("click", () => {
    removeFacture();
    window.setTimeout(() => {
  ...
  response.querySelector(".cancel").addEventListener("click", () => {
    removeFacture();
    window.setTimeout(() => {
      document.body.removeChild(response);
      renderFormFacture();
  ...
  response.querySelector(".confirm").addEventListener("click", async () => {
    let form = new FormData();
    form.append("facture", JSON.stringify(facture));
    await get("http://localhost:120/SI/hello/confirm_facture", "POST", form);
    removeFacture();
    window.setTimeout(() => {
      document.body.removeChild(response);
      renderFormFacture();
